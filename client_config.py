...
class client_config:
    ...
    remote_rpcurl = "http://:8545"  # 采用rpc通信时，节点的rpc端口,和要通信的节点*必须*一致,如采用channel协议通信，这里可以留空
    channel_host = ""  # 采用channel通信时，节点的channel ip地址,如采用rpc协议通信，这里可以留空
    ...
    solcjs_path = "./solcjs"
    config_port = 5081
    fiscobcos_abi_file = "contracts/evidence.abi"  # 上链智能合约abi路径
    fiscobcos_bin_file = "contracts/evidence.bin"  # 上链智能合约bin路径
